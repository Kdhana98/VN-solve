...
# This file is for evaluating the model with different node_size and edge_width
...
if visualization == 'ellipse':
    tempt = str(vertical).replace('.', 'p')
if node_size != 0.5:
    tempt = 'node_size_'+str(node_size).replace('.', 'p')+'_' + tempt
if edge_width != 0.1:
    tempt = 'edge_width_'+str(edge_width).replace('.', 'p')+'_' + tempt
...
for s in [3, 7, 11, 13, 29]:
    if resize == True:
        ...
        if tempt != '':
            test_dir = tempt +'_'+ coloring + '_color_' + visualization + '/data_' + tempt + '_' + coloring + '_' + visualization + '_' + model_size + '_' + str(
    ...
    if resize == True:
        ...
        if tempt != '':
            model.load_state_dict(torch.load(
                    'best_model_'+ tempt + '_' + coloring + '_' + visualization + '_' + model_size + '_' + str(
